function solution(A, X) {
    let num, sets = [], w, h, notDone;
    if(A.length == 0) {
        return 0
    }
    
    A = A.filter((val, index) => A.indexOf(val) != A.lastIndexOf(val));
    
    for(let i = 0; i<A.length - 1; i++) {
        for(let j = i+1; j<A.length; j++ ) {
            w = A[i], h = A[j];
            notDone = !sets.includes(`${w},${h}`) && !sets.includes(`${h},${w}`);
            
            if(notDone) {
                
                if(w !=h || (w == h && A.filter(val => val == w).length >=4)) {
    ...
    // if(sets.length > 2) {
    //     sets = sets.map(val => val.sort((a,b) => a - b));
    //     // sets = sets.filter((val, index) => index == sets.indexOf(val))
    // }
